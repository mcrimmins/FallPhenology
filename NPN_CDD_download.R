# NPN XML download, ACIS Cooling Degree Day
# MAC 05/15/20
...
dataQ$firstDate<-as.Date(paste0(dataQ$first_yes_year,"-",dataQ$first_yes_month,"-",dataQ$first_yes_day),  format = "%Y-%m-%d")

for(i in 1:nrow(dataQ)){
  lat<-dataQ$latitude[i]
  lon<-dataQ$longitude[i]
  base<-77 # SET base temperature, 25C/77F for Acer rubrum 
  sdate<-paste0(dataQ$first_yes_year[i],"-07-01") # SET to -01-01 for breaking leaf buds, -08-01
...
attr(dataQ,"CDD_base_F" )<-base

save(dataQ, file="NPN_CDD_498_sugarmaple_base77F_052220.Rdata")
